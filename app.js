...
(function () {
  ...
  function renderSettings() {
    ...
    ${window.ShredBackend && window.ShredBackend.configured ? `<div class="card"><h3>Compte</h3>${window.ShredBackend.connected ? `<p class="small muted" style="margin-top:6px">Connecté : ${esc(window.ShredBackend.user ? window.ShredBackend.user.email : "")}</p><button class="btn ghost sm" style="margin-top:8px" data-act="logout">Se déconnecter</button>` : `<p class="small muted" style="margin:6px 0 10px">Identifiants du compte Supabase (créé une seule fois).</p><div class="stack"><input type="email" id="login-email" placeholder="Email" autocomplete="username"><input type="password" id="login-pw" placeholder="Mot de passe" autocomplete="current-password"><button class="btn primary" data-act="login">Se connecter</button></div>`}</div>` : ""}
    ...
    <div class="card"><h3>Diagnostic</h3><p class="tiny muted" style="margin-top:6px">Build 4 · ${Store.db ? "base connectée" : "base non connectée"} · ${Store.pending.size} en attente · écran ${window.innerWidth}×${window.innerHeight}${UI.lastError ? " · dernière erreur : " + esc(UI.lastError) : ""}</p></div>
  ...
  document.addEventListener("click", async (ev) => {
    ...
    switch (act) {
      ...
      case "login": { try { await window.ShredBackend.login($("#login-email").value.trim(), $("#login-pw").value); toast("Connecté"); await Store.connectSupabase(); Store.updateBanner(); render(); } catch (e) { toast("Connexion refusée : " + e.message); } break; }
